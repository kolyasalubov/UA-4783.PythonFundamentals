...
while run:
    ...
    current_width = gameDisplay.get_width()
    current_height = gameDisplay.get_height()
    
    if keys[pygame.K_LEFT] and COORD_X > 0:
        COORD_X = COORD_X - DELTA_STEP
    if keys[pygame.K_RIGHT] and COORD_X < current_width - WIDTH_RECTANGLE:
        COORD_X = COORD_X + DELTA_STEP
    if keys[pygame.K_UP] and COORD_Y > 0:
        COORD_Y = COORD_Y - DELTA_STEP
    if keys[pygame.K_DOWN] and COORD_Y < current_height - HEIGHT_RECTANGLE:
        COORD_Y = COORD_Y + DELTA_STEP
